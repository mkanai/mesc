        ...
        for i, l in enumerate(f):
            ...
            if i == 0:
                ...
                if curr_gene == prev_gene:
                    ...
                    if prev_chr in chr:
                        raise ValueError(
                            "Chromosomes not sorted. eQTLs on each chromosome must be in one contiguous block."
                        )
                    chr.add(prev_chr)
                    each_chrom[prev_chr - 1] += 1
    return each_chrom, i
...
def estimate_expression_cis_herit(ref_ld, frq, zscores, ref_ld_indices):
    ...
    if np.array_equal(ref_ld.iloc[start_snp_idx : end_snp_idx + 1, 1].values, zscores["SNP"].values):
        ...
        hsqhat = reg.Hsq(
            s(zscores["Z"].values ** 2),
            s(temp_ref_ld.iloc[:, 3]),
            s(temp_ref_ld.iloc[:, 3]),
            s(zscores["N"]),
            M_annot,
            n_blocks=10,
    ...
    with open_file(cismat) as f:
        for i, line in enumerate(f):
            ...
            if i == 0:
                ...
                chrom_gene_num += 1
                print(
                    "Read gene {} of {} ({} of {} total): {}".format(
                        chrom_gene_num, n_genes[prev_chr - 1], gene_num, sum(n_genes), prev_gene
                    )
                )
                ...
                if len(temp_gene_mat) <= 10:
                    ...
                    temp_gene_mat["EFF"] = (
                        temp_gene_mat["Z"].values ** 2 / temp_gene_mat["N"].values - 1 / temp_gene_mat["N"].values
                    )
                ...
                if args.gene_sets is not None:
                    ...
                    for k in gsets.keys():
                        gset_names.extend(
                            ["{}_Cis_herit_bin_{}".format(k, x) for x in range(1, args.num_gene_bins + 1)]
                        )

                    # create dict indicating membership of each gene in each gene set
                    gene_gset_dict = defaultdict(list)
                    gene_bins = pd.qcut(
                        np.array(eqtl_herits), args.num_bins, labels=range(1, args.num_bins + 1)
                    ).astype(int)
                    all_gset_gene_bins = pd.qcut(
                        np.array(eqtl_herits), args.num_gene_bins, labels=range(1, args.num_gene_bins + 1)
                    ).astype(int)
                    ...
                    for k, v in gsets.items():
                        ...
                        if len(v) > 0:
                            ...
                            if len(v) == 1:
                                ...
                                gset_gene_bins = pd.qcut(
                                    np.array(eqtl_herits)[gene_indices],
                                    args.num_gene_bins,
                                    labels=range(1, args.num_gene_bins + 1),
                                ).astype(int)
                    ...
                    for j in range(0, len(all_summ)):
                        ...
                        if np.array_equal(
                            snps.iloc[start_snp_idx : end_snp_idx + 1, 1].values, all_summ[j][2]["SNP"].values
                        ):
                            for gix in gset_idx:
                ...
                g_annot_final.to_csv(
                    "{}.{}.gannot.gz".format(args.out, prev_chr), sep="\t", index=False, compression="gzip"
                )

                all_herit = pd.DataFrame.from_records(
                    all_herit, columns=["Gene", "Chrom", "h2cis", "h2cis_se", "h2cis_p"]
                )
                ...
                expscore.to_csv(
                    "{}.{}.expscore.gz".format(args.out, prev_chr),
                    sep="\t",
                    index=False,
                    compression="gzip",
                    float_format="%.5g",
